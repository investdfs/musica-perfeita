    ...
    <div className="min-h-screen flex flex-col bg-gray-50 text-gray-900">
      ...
      <main className="flex-grow container mx-auto px-3 sm:px-4 py-6 sm:py-8 pb-24">
        <section className="mb-10 bg-white p-6 rounded-lg shadow-sm">
          <h2 className="text-2xl font-semibold mb-4 text-center text-purple-800">{t("ourMusic.aboutTitle")}</h2>
          <div className="grid md:grid-cols-3 gap-6 mb-8">
            <div className="bg-purple-50 p-5 rounded-lg">
              <h3 className="font-medium text-lg mb-2 text-purple-700">{t("ourMusic.uniqueCreation")}</h3>
              <p className="text-gray-700">
                {t("ourMusic.uniqueDesc")}
              </p>
            </div>
            <div className="bg-pink-50 p-5 rounded-lg">
              <h3 className="font-medium text-lg mb-2 text-pink-700">{t("ourMusic.advancedTech")}</h3>
              <p className="text-gray-700">
                {t("ourMusic.techDesc")}
              </p>
            </div>
            <div className="bg-yellow-50 p-5 rounded-lg">
              <h3 className="font-medium text-lg mb-2 text-yellow-700">{t("ourMusic.styleVariety")}</h3>
              <p className="text-gray-700">
                {t("ourMusic.styleDesc")}
              </p>
            </div>
          </div>
        </section>
        ...
        <section className="max-w-4xl mx-auto mt-12 text-center">
          <h3 className="text-2xl font-semibold mb-4 text-purple-800">
            {t("ourMusic.readyTitle")}
          </h3>
          <p className="text-gray-700 mb-6 max-w-2xl mx-auto">
            {t("ourMusic.readyDesc")}
          </p>
          <Link to="/cadastro">
            <Button size="lg" className="bg-purple-600 hover:bg-purple-700">
              {t("ourMusic.createButton")} <ArrowRight className="ml-2 h-4 w-4" />
            </Button>
          </Link>
        </section>
      </main>
